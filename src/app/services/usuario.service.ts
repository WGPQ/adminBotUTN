...
export class UsuarioService {
  ...
  // SERVICIO USUARIO
  obtenerUsuarios(rol: string, listar: Listar): Observable<Usuario[]> {
  ...
  obtenerUsuario(id?: string): Observable<Usuario> {
    if (id == undefined) {
      var decrypted = crypto.AES.decrypt(this.authservices.leerIdUserRol(), this.secret);
      id = decrypted.toString(crypto.enc.Utf8);
    }

    return this.http.get(this.appUrl + this.apiUsuario + '/Obtener/' + id, { headers: this.headers }).pipe(
      map((resp: any) => {
        if (resp.exito) {
          return resp.data;
        } else {
  ...
  ingresarUsuario(usuario: Usuario): Observable<any> {
    return this.http.post(this.appUrl + this.apiUsuario + '/Insertar', usuario, { headers: this.headers });
  }
  eliminarUsuario(id?: string): Observable<any> {
    return this.http.delete(this.appUrl + this.apiUsuario + '/Eliminar/' + id, { headers: this.headers })
  }
  actualizarUsuario(usuario: Usuario): Observable<any> {
    return this.http.post(this.appUrl + this.apiUsuario + '/Actualizar', usuario, { headers: this.headers });
  }
  ...
  exportarExcel(rol: string, listar: Listar): Observable<any> {
    ...
    const params = new HttpParams()
      ...
      .set("limit", listar.limit)
      .set("sort", listar.sort)
    return this.http.get(this.appUrl + this.apiUsuario + '/Exportar', { params: params, headers: headers, responseType: 'blob' });
